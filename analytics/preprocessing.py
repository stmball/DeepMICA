...
class PPData:
    ...
    def __init__(self, x_data, y_data, scaler, sampleRate, batch_size, max_states, stateDict):
        self.x_data = x_data
        self.y_data = y_data
        self.scaler = scaler
...
def MICAProcess(data, state_column, current_column, max_states, batch_size, sampleRate, stateDict, tts=False):
    ...
    if tts:
        ...
        trainSize, testSize = len(xtrain)//batch_size * \
            batch_size, len(xtest)//batch_size * batch_size
        ...
        xtrain = xtrain.reshape((xtrain.shape[0]//batch_size, batch_size, 1))
        xtest = xtest.reshape((xtest.shape[0]//batch_size, batch_size, 1))
        ytrain = ytrain.reshape(
            (ytrain.shape[0]//batch_size, batch_size, max_states))
        ytest = ytest.reshape(
            (ytest.shape[0]//batch_size, batch_size, max_states))

        return PPData(x_data=(xtrain, xtest), y_data=(ytrain, ytest), scaler=(minmaxTrain, minmaxTest), sampleRate=sampleRate, batch_size=batch_size, max_states=max_states, stateDict=stateDict)
        ...
        dataSize = len(xdata)//batch_size * batch_size
        ...
        xdata = xdata.reshape((xdata.shape[0]//batch_size, batch_size, 1))
        ydata = ydata.reshape(
            (ydata.shape[0]//batch_size, batch_size, max_states))

        return PPData(x_data=xdata, y_data=ydata, scaler=minmax, sampleRate=sampleRate, batch_size=batch_size, max_states=max_states, stateDict=stateDict)


def PPFromFile(data_filepath, state_column, current_column, max_states, batch_size, stateDict, tts=False):
    """
    ...
    return MICAProcess(data, state_column, current_column, max_states, batch_size, sampleRate, stateDict, tts=tts)


def PPFromMarkovLog(MarkovLog, batch_size, max_states, tts=False):
    ...
    if not isinstance(MarkovLog.continuous_history, pd.DataFrame):
        ...
        return MICAProcess(data=MarkovLog.continuous_history, state_column="State", current_column="Noisy Current", max_states=max_states, batch_size=batch_size, sampleRate=MarkovLog.sample_rate, stateDict=MarkovLog.state_dict, tts=tts)
